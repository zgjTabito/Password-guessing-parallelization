...
int main()
{
    ...
    // ---------------- CUDA 初始化隔离器 ----------------
    cout << "[INFO] Forcing CUDA initialization..." << endl;
    auto start_cuda = system_clock::now();
    cudaFree(0);  // 触发 CUDA runtime 的初始化
    auto end_cuda = system_clock::now();
    auto duration_cuda = duration_cast<microseconds>(end_cuda - start_cuda);
    double time_cuda_init = double(duration_cuda.count()) / 1e6;
    cout << "[INFO] CUDA init time: " << time_cuda_init << " seconds" << endl;
    // ---------------------------------------------------
    ...
    {
        auto pts = q.PopBatchAndSchedule(16); // 保留新PT逻辑，取出用于并行生成的 PT
        vector<string> batch_guesses;

        GeneratePTGuessesBatchCUDA(pts, q.m, batch_guesses); // GPU kernel 并行生成猜测
