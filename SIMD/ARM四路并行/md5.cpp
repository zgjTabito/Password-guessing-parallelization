 ...
 */
 void MD5Hash(string* inputs, uint32x4_t* state) {
    Byte *paddedMessage[4];   // 用于存储四个输入口令的填充信息
    int *messageLength=new int[4];      // 每个输入口令的长度
    uint32x4_t x[16];          // 存储每个数据块的SIMD向量
	for (int i = 0; i < 4; i++) {
    ...
    for (int i = 0; i < n_blocks; i++) {
        // 加载每个数据块的值到SIMD寄存器中
        for (int j = 0; j < 16; j++) {
			uint32_t vals[4]; // 用于存储四个口令的当前块值
			for (int k = 0; k < 4; k++) {
				vals[k] = static_cast<uint32_t>(
			...
			// 将四个口令的值组合到一个 SIMD 寄存器中
			x[j] = vsetq_lane_u32(vals[0], x[j], 0); // 第一个维度由第一个口令决定
			x[j] = vsetq_lane_u32(vals[1], x[j], 1); // 第二个维度由第二个口令决定
			x[j] = vsetq_lane_u32(vals[2], x[j], 2); // 第三个维度由第三个口令决定
			x[j] = vsetq_lane_u32(vals[3], x[j], 3); // 第四个维度由第四个口令决定
		}
...
// 替换原有的 shuffle 代码
for (int i = 0; i < 4; i++) {
    uint8x16_t tmp = vreinterpretq_u8_u32(state[i]);
    tmp = vrev32q_u8(tmp);
    state[i] = vreinterpretq_u32_u8(tmp);
}
