...
int main()
{
    ...
    while (!q.priority.empty())
    {
        ...
        if (curr_num > 1000000)
        {
            ...
            uint32x4_t state1[4]; // 用于存储前4个 SIMD MD5 的结果
            uint32x4_t state2[4]; // 用于存储后4个 SIMD MD5 的结果
            vector<string> batch; // 用于存储每次处理的8个口令
            
            for (size_t i = 0; i < q.guesses.size(); i++) {
                batch.push_back(q.guesses[i]);
            
                // 当 batch 中有 8 个口令时，调用 SIMD MD5Hash 函数
                if (batch.size() == 8) {
                    MD5Hash(batch.data(), state1, state2);
