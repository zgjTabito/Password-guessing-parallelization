 ...
 */
 void MD5Hash(string* inputs, uint32x2_t* state) {
    Byte *paddedMessage[2];   // 用于存储两个输入口令的填充信息
    int *messageLength = new int[2];    // 每个输入口令的长度
    uint32x2_t x[16];         // 存储每个数据块的SIMD向量
    ...
    for (int i = 0; i < n_blocks; i++) {
        ...
        for (int j = 0; j < 16; j++) {
            ...
            // 将两个口令的值组合到一个 SIMD 寄存器中
            x[j] = vset_lane_u32(vals[0], x[j], 0); // 第一个维度由第一个口令决定
            x[j] = vset_lane_u32(vals[1], x[j], 1); // 第二个维度由第二个口令决定
        }
