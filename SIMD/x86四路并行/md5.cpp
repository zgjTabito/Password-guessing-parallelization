...
#include <cassert>
#include <chrono>
#include <immintrin.h> // 包含 SSE/AVX 指令集
#include <iomanip>
#include <iostream>
#include <string>
...
for (int i = 0; i < 4; i++) {
  ...
  for (int i = 0; i < n_blocks; i++) {
    for (int j = 0; j < 16; j++) {
      ...
      for (int k = 0; k < 4; k++) {
        vals[k] =
            static_cast<uint32_t>((paddedMessage[k][4 * j + i * 64]) |
                                  (paddedMessage[k][4 * j + 1 + i * 64] << 8) |
                                  (paddedMessage[k][4 * j + 2 + i * 64] << 16) |
                                  (paddedMessage[k][4 * j + 3 + i * 64] << 24));
      }
      x[j] = _mm_set_epi32(vals[3], vals[2], vals[1], vals[0]);
    }
  ...
  for (int i = 0; i < 4; i++) {
    __m128i tmp =
        _mm_shuffle_epi8(state[i], _mm_set_epi8(12, 13, 14, 15, 8, 9, 10, 11, 4,
                                                5, 6, 7, 0, 1, 2, 3));
    state[i] = tmp;
  }
